const chai = require('chai');
const chaiHttp = require('chai-http');
const { app, server } = require('../express_server.js');
const expect = chai.expect;

chai.use(chaiHttp);

describe('express_server.js tests\n', () => {
  describe('Testing login and accessing restricted URL', () => {
    let agent = chai.request.agent(app);

    it('should login successfully and then access restricted URL with a status code of 403', () => {
      // Register a new user
      return agent
        .post('/register')
        .send({ email: 'test@example.com', password: 'test123' })
        .then(res => {
          expect(res).to.redirect; // Assuming successful registration redirects to login page

          // Login with the newly registered user credentials
          return agent
            .post('/login')
            .send({ email: 'test@example.com', password: 'test123' });
        })
        .then(res => {
          expect(res).to.redirect; // Assuming successful login redirects to /urls page

          // Access the restricted URL after successful login
          return agent.get('/urls/b6UTxQ');
        })
        .then(res => {
          // Expecting status code 403 since the URL is restricted
          expect(res).to.have.status(403);
        })
        .catch(err => {
          throw err; // Throw any errors encountered during the test
        });
    });
  });

    describe('Redirecting GET requests', () => {
      it('GET request to / should redirect to /login with a status code of 302 when not logged in', (done) => {
        chai.request(app)
          .get('/')
          .redirects(0)
          .end((err, res) => {
            expect(res).to.redirectTo('/login');
            expect(res).to.have.status(302);
            done();
          });
      });

      it('GET request to /urls/new should redirect to /login with a status code of 302 when not logged in', (done) => {
        chai.request(app)
          .get('/urls/new')
          .redirects(0)
          .end((err, res) => {
            expect(res).to.redirectTo('/login');
            expect(res).to.have.status(302);
            done();
          });
      });

      it('GET request to /urls/NOTEXISTS should receive a status code of 404', (done) => {
        chai.request(app)
          .get('/urls/NOTEXISTS')
          .redirects(0)
          .end((err, res) => {
            expect(res).to.have.status(404);
            done();
          });
      });

      it('GET request to /urls/NOTEXISTS should receive a status code of 404', (done) => {
        chai.request(app)
          .get('/urls/NOTEXISTS')
          .redirects(0)
          .end((err, res) => {
            expect(res).to.have.status(404);
            done();
          });
      });

      it('GET request to /urls/b6UTxQ should receive a status code of 403', (done) => {
        chai.request(app)
          .get('/urls/b6UTxQ')
          .redirects(0)
          .end((err, res) => {
            expect(res).to.have.status(403);
            done();
          });
      });
    });

  after((done) => {
    server.close((err) => {
      if (err) {
        console.error('\nError closing server:', err);
      } else {
        console.log('\nServer closed successfully');
      }
      done(err); 
    });
  });
});
